...
test('all v0.7.4 release metadata agrees exactly', async () => {
  ...
  assert.match(openapi, /^\s*version:\s*0\.7\.4\s*$/m);
  assert.match(health, /version:\s*['"]0\.7\.4['"]/);
  assert.match(smoke, /\.version\s*==\s*"0\.7\.4"/);
  assert.match(smoke, /v0\.7\.4 production deploy/);
