...
import { isVerifiedOfficialDomain } from '../official-domain.ts';
import type {
  DiscoveryCandidate,
  MarketProvider,
...
function requireDefinition() {
  const value = providerDefinitionById('official');
  if (!value) throw new Error('Official provider definition is missing');
  return value;
}
...
export const officialProvider: MarketProvider = {
  id: definition.id,
  ...
  extractOffer(verified, context) {
    if (!isVerifiedOfficialDomain(verified.page.url, context.target)) return null;
    ...
    return {
      ...offer,
      id: `${definition.market}:${offer.url}`,
      market: definition.market,
    };
  },
};
